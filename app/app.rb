module GithubIssues
  class App < Padrino::Application
    ...
    get "/" do
      html = ['<ul>']
      self.class.routes.map do |route|
        html << "<li><a href=#{route.path}>#{route.name}</a></li>" if route.verb == 'GET' && route.name
      end
      html << '</ul>'
      render :slim, html.join
    end
    ...
    #
    # set :cache, Padrino::Cache.new(:LRUHash) # Keeps cached values in memory # default choice
